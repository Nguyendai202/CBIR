...
class HOG(object):
  ...
  def histogram(self, input, n_bin=n_bin, type=h_type, n_slice=n_slice, normalize=True):
    ...
    if type == 'global':# tính HOG cho toàn bộ ảnh 
      hist = self._HOG(img, n_bin)
  
    elif type == 'region':# Tính HOG cho từng vùng ảnh 
  ...
  def _HOG(self, img, n_bin, normalize=True):
    ...
    fd = hog(image, orientations=n_orient, pixels_per_cell=p_p_c, cells_per_block=c_p_b)# chưa chuẩn hoá , nếu chuẩn hoá thì đặt normalize = true
    bins = np.linspace(0, np.max(fd), n_bin+1, endpoint=True)
    hist, _ = np.histogram(fd, bins=bins)
  
    if normalize:
      hist = np.array(hist) / np.sum(hist)# tự chuẩn hoá 
  ...
  def make_samples(self, db, verbose=True):
    ...
    try:
      ...
      for d in data.itertuples():
        ...
        d_hist = self.histogram(d_img, type=h_type, n_slice=n_slice)# tính HOG 
        samples.append({
                        'img':  d_img, 
                        'cls':  d_cls, 
                        'hist': d_hist
