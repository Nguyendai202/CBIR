...
stride = (1, 1)
n_slice  = 10# chia hình ảnh thành 10 ô vuông 
h_type   = 'region'
d_type   = 'cosine'
...
class Edge(object):
  ...
  def histogram(self, input, stride=(2, 2), type=h_type, n_slice=n_slice, normalize=True):
    ...
    hist = 0
    if type == 'global':# tính toán histogram cho toàn bộ ảnh
      hist = self._conv(img, stride=stride, kernels=edge_kernels)
    elif type == 'region':# tính toán histogram cho từng ô vuông
  ...
  def _conv(self, img, stride, kernels, normalize=True):
    H, W, C = img.shape
    conv_kernels = np.expand_dims(kernels, axis=3)# mở rộng chiều , theem 1 vào sau vd: (2,3,4,1)
    conv_kernels = np.tile(conv_kernels, (1, 1, 1, C))# tạo magj mới lặp lại mảng cũ theo (1,1,1,C)= độ dài C
  ...
  def make_samples(self, db, verbose=True):# lẩ ra một số mẫu từ db đc lưu sẵn  và đã trích xuất đặc trưng 
    #verbose=True: sẽ in thông báo ra màn hình
    if h_type == 'global':
      sample_cache = "edge-{}-stride{}".format(h_type, stride)
    elif h_type == 'region':
      sample_cache = "edge-{}-stride{}-n_slice{}".format(h_type, stride, n_slice)
  
    try:# thử load lên nếu có 
      ...
      for d in data.itertuples():
        d_img, d_cls = getattr(d, "img"), getattr(d, "cls")# gán giá trị củra 2 thuộc tính img,cls vào biến
      ...
      cPickle.dump(samples, open(os.path.join(cache_dir, sample_cache), "wb", True))# lưu vào cache để sau dùng 
