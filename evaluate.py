...
from scipy import spatial# cung cấp công cụ cho tính toán kc , xây dựng cây kd-tree , tối ưu hoá truy vấn ko gian
import numpy as np
...
def AP(label, results, sort=True):# tính toán độ đo AP(đôh chính xác trung bình của precision)
  ...
  for i, result in enumerate(results):
    if result['cls'] == label:
      hit += 1
      precision.append(hit / (i+1.))# số lượng mẫu đúng trong i mẫu đầu (mỗi giá trị)
  if hit == 0:
    return 0.
...
def evaluate_class(db, f_class=None, f_instance=None, depth=None, d_type='d1'):
  ''' infer the whole database
    ...
    arguments
      ...
      depth  : retrieved depth during inference, the default depth is equal to database size(số lượng 
  ...
  if f_class:
    ...
    f = f_instance
  samples = f.make_samples(db)# lấy ra các mẫu từ db 
  for query in samples:
    ap, _ = infer(query, samples=samples, depth=depth, d_type=d_type)
    ret[query['cls']].append(ap)
